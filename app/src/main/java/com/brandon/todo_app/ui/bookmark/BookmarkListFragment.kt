...
import com.brandon.todo_app.databinding.BookmarkListFragmentBinding
...
class BookmarkListFragment : Fragment() {
            ...
            if (result.resultCode == Activity.RESULT_OK) {
                ...
                val entryType = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    result.data?.getSerializableExtra(
                        TodoContentConstant.EXTRA_TODO_CONTENT_ACTION_TYPE,
                        TodoContentActionType::class.java
                    )
                } else {
                    result.data?.getSerializableExtra(TodoContentConstant.EXTRA_TODO_CONTENT_ACTION_TYPE) as TodoContentActionType
                }

                val entity = IntentCompat.getParcelableExtra(
                    result.data ?: Intent(),
                    TodoContentConstant.EXTRA_TODO_ENTITY,
    ...
    private fun initViewModel() {
        with(sharedViewModel){
            todoItemList.observe(viewLifecycleOwner) { todoList ->
                // isBookmarked = true 인 아이템만 필터링 하여 전달 구현
                val bookmarkedList = todoList.filter { it.isBookmarked }
                listAdapter.submitList(bookmarkedList)
            }
        }
        with(viewModel){
